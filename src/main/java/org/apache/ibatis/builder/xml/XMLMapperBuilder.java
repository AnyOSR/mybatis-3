...
public class XMLMapperBuilder extends BaseBuilder {
  ...
  private void configurationElement(XNode context) {
    try {
      ...
      cacheElement(context.evalNode("cache"));          // 解析cache
  ...
  private void buildStatementFromContext(List<XNode> list, String requiredDatabaseId) {
    // 遍历所有的sql  select|insert|update|delete
  ...
  private void cacheRefElement(XNode context) {
    if (context != null) {
      // 将当前currentNameSpace的cache应用 refer到别的一个namespace
  ...
  // resultMap子节点的子节点可能会有nestResultMap
  // 这些子节点或者有resultMap属性，或者是association collection case元素，且没有select属性
  ...
  private Discriminator processDiscriminatorElement(XNode context, Class<?> resultType, List<ResultMapping> resultMappings) throws Exception {
    ...
    for (XNode caseChild : context.getChildren()) {                   // value resultMap
  ...
  private void bindMapperForNamespace() {
    ...
    if (namespace != null) {
      ...
      try {
        boundType = Resources.classForName(namespace);  // 查找nameSpace对应的class
